...
// Dispara scripts/video/scrape-ugc.ts para coletar vídeos TikTok via Apify.
//
...
export async function POST(
  ...
  try {
    ...
    const max   = Math.min(parseInt(body.max ?? '20', 10), 50)
    ...
    // process.cwd() = frontend/ em dev; o projeto raiz fica um nível acima
    const projectRoot = path.join(process.cwd(), '..')
    const scriptPath  = path.join(projectRoot, 'scripts', 'video', 'scrape-ugc.ts')

    const result = await runScript(projectRoot, scriptPath, productId, query, max)
...
function runScript(
  ...
  return new Promise((resolve) => {
    const child = spawn(
      'npx',
      ['tsx', scriptPath, '--product-id', productId, '--query', query, '--max', String(max)],
      {
        cwd,
        env: { ...process.env },
    ...
    child.on('close', (code) => {
      ...
      const match = stdout.match(/(\d+) vídeos salvos/)
